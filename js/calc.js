		...
		var pontcount = 0; //contban szereplő műveletjelek száma
			...
			function send(me){ //gomb funkciók
				document.querySelectorAll("a").ondragstart = function() { return false; }; //ne lehessen áthuzni a hivatkozást
				...
				if (Number.isInteger(parseInt(szam))){ //számok bevitele
					...
					if(szam == "0"){
						if(cont.slice(-1) == "0" && cont.slice(cont.length-2,-1) !="." && isNaN(cont.slice(cont.length-2,-1)) || cont.length == 1 && cont.slice(-1) == "0"){} //hogy ne ismétlődjön a nulla
						else{
							cont = cont + szam;
							addtofield();
						}
					...
					if(szam == "."){//pont bevitele
						if(pontcount == 1){} //ha a pontcount 1 akkor nem visz be pontot
						else{
							cont = cont + szam;
							addtofield();
							csere = 0;
					...
					if(szam == "." && String(cont).indexOf('.') > -1){} //pontot ne cseréljen le műveletijelre
					else{
